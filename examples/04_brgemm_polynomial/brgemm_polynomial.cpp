...
int gemm_polynomial_result_validate(data_type_a *A_device,
    ...
    bool result = buff_cmp::xetla_buff_cmp(
            data, other, "brgemm_polynomial validation");
...
void brgemm_polynomial_run(int iter) {
    ...
    profiling_helper prof("brgemm_polynomial", ops, "gflops");
...
int main() {
    ...
    // be applied to the result of BRGEMM, here provides some example:
    // - BRGEMM
    //   C  = A x B
    // - tile_op_t=relu_op_t
    //   C = ReLU(A x B)
