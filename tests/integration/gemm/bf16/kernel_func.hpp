...
template <int wg_num_m_, gpu_arch arch_tag_>
struct group_swizzle_m_first {
public:
...
template <typename dtype_a, typename dtype_b, typename dtype_c,
        ...
        mem_layout layout_b, uint32_t global_kslicing, uint32_t local_kslicing,
        mma_engine engine>
struct bf16_gemm_test_func {
    using tile_shape = tile_shape_t<wg_n, wg_m, sg_n, sg_m>;
    static constexpr uint32_t periodic_sync_interval = 8;
    static constexpr uint32_t prefetch_distance = 3;
    using gemm_t = typename gemm_selector_t<dtype_a, dtype_b, layout_a,
