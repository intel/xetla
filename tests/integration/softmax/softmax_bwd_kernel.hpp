...
template <typename dtype_in, typename dtype_coff_in, typename dtype_out,
        typename dtype_acc, uint32_t wg_n, uint32_t wg_m, uint32_t sg_n,
        uint32_t sg_m>
struct softmax_bwd_test_func {
    ...
    using softmax_bwd_t = group::softmax_t<
            group::softmax_policy_bwd<dtype_coff_in, dtype_acc, gpu_arch::Xe>,
            tile_shape>;
    static constexpr uint32_t barrier_count
            = softmax_bwd_t::get_barrier_count::count;
    static constexpr uint32_t slm_size = softmax_bwd_t::get_slm_size::size;
    using softmax_bwd_args_t = typename softmax_bwd_t::arguments_t;

    static const char *func_name() { return "softmax_bwd_test_func"; }

    static inline void run(sycl::nd_item<3> &item, dtype_in *mat_in_ptr,
            dtype_coff_in *mat_coff_ptr, dtype_out *mat_out_ptr, uint32_t mat_m,
            uint32_t mat_n, uint32_t mat_ld, dtype_acc sqrt_dk_inv) {
        ...
        softmax_bwd_t softmax_bwd;
        softmax_bwd_args_t softmax_bwd_args(
                {mat_coff_ptr}, {boundary_n, boundary_m, mat_ld}, sqrt_dk_inv);
        softmax_bwd(g, matAcc, {start_n, start_m}, softmax_bwd_args);
