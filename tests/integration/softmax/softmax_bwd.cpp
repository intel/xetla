...
void softmax_bwd_run() {
    ...
    auto buffer_coff_in_host = alloc_host_and_copy<data_type_coff_in>(
            buffer_coff_in, size_in, queue);
    auto buffer_out_host
            = alloc_host_and_copy<data_type_out>(buffer_out, size_out, queue);

    ASSERT_EQ(0,
            (bwd_reduction_result_validate<data_type_in, data_type_coff_in,
                    data_type_out, data_type_acc>(buffer_in_host,
                    buffer_coff_in_host, buffer_out_host, mat_m, mat_n,
                    sqrt_dk_inv)));
