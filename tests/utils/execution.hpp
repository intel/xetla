...
void gemm_exec(const std::string &compile_str, size_t batch = 1) {
    ...
    size_t pingpong_size_a = max(
            Test::mat_m * Test::mat_k, l3_cache_size / sizeof(data_type_a));
    size_t pingpong_size_b = max(
            Test::mat_n * Test::mat_k, l3_cache_size / sizeof(data_type_b));
    size_t pingpong_size_c = max(
            Test::mat_m * Test::mat_n, l3_cache_size / sizeof(data_type_c));
#else
    ...
    auto A = alloc_device_and_init<data_type_a>(
            pingpong_size_a, mem_iter * batch,
            [](data_type_a *data, size_t idx) {
                data[idx] = static_cast<data_type_a>(random_float());
            },
            queue, device, context);
    auto B = alloc_device_and_init<data_type_b>(
            pingpong_size_b, mem_iter * batch,
            [](data_type_b *data, size_t idx) {
                data[idx] = static_cast<data_type_b>(random_float());
            },
            queue, device, context);
    auto C = alloc_device_and_init<data_type_c>(
            pingpong_size_c, mem_iter * batch,
            ...
            auto e_esimd = queue.submit([&](handler &cgh) {
                ...
                cgh.parallel_for(nd_range, [=](nd_item<3> item) KERNEL_MAIN {
                    ...
                    auto Cnt_ptr = Cnt + (it * batch + batch_idx) * size_cnt;
                            ...
                            auto B_ptr = B + batch_idx * size_b + it * pingpong_size_a;
                            auto C_ptr = C + batch_idx * size_c + it * pingpong_size_a;
