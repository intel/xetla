...
bool explorer::tune_kernel_one_by_one(cmdline_input_info &cmd_info,
    ...
    std::tm *tm;
    for (const auto &one_op_fuse_elem : all_op_cart_proc) {

        tune_dbg_rec->write_log("---Start one round tune kernel: "
                + get_cur_time() + "---\n" + "micro_kernel:");
        for (const auto &mk_name : one_op_fuse_elem) {
        ...
        for (const auto &mk_name : one_op_fuse_elem) {
            ...
            for (const auto &para_name : mk_attr_map[mk_name].tune_attr) {
        ...
        for (const auto &one_piece_of_tune_para : tune_para_set) {
...
void explorer::parse_thread_pool_exec_result(
    ...
    for (auto &&cur_res : final_build_res) {
        ...
        for (const auto &cur_tune_para_name : get_tune_para_name_list()) {
...
void explorer::tune_kernel(cmdline_input_info &cmd_info,
    ...
    for (const auto &op : tune_mk_set) {
        vector<string> all_mk_in_one_op;
        for (const auto &mk : op) {
            ...
            for (const auto &tune_para : mk.tune_attr) {
                vector<string> para = {};
...
void explorer::print_cart_proc(vector<vector<string>> cart_res) {
    for (const auto &elem : cart_res) {
        cout << endl;
        for (const auto &st : elem) {
            cout << st << ", ";
        }
    }
    cout << endl;
...
void explorer::range_to_vector(
        const tune_attr_cfg &tune_cfg, vector<string> &res) {
    const range_element_attr &range_attr = tune_cfg.attr;
    const std::string &attr_name = tune_cfg.attr_name;
    for (auto i = range_attr.start_value; i <= range_attr.end_value;
            i += range_attr.stride) {
        // if ("local_kslicing" == attr_name)
        {
            if ((i & (i - 1)) != 0) { continue; }
            std::cout << "name: " << tune_cfg.attr_name << ", value: " << i
                      << endl;
        }
...
bool explorer::check_is_valid_cfg(
    ...
    // if (((wg_m / sg_m) * (wg_n / sg_n) * local_kslicing) != 32) {
    std::cout << "wg_m:" << wg_m << ",wg_n:" << wg_n << ",line:" << __LINE__
              << std::endl;
    std::cout << "sg_m:" << sg_m << ",sg_n:" << sg_n << ",line:" << __LINE__
              << std::endl;
    std::cout << "local_kslicing:" << local_kslicing << ",line:" << __LINE__
              << std::endl;
    //       return false;
    //}

    uint32_t sg_k = stoi(input_tune_para_set.at("sg_k"));
    std::map<string, uint32_t> data_type_size = {
            {"bf16", 2}, {"fp16", 2}, {"float", 4}, {"tf32", 4}, {"int8", 1}};
    ...
    // for (const auto& para :tuning_params) {
