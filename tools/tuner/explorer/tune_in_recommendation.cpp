...
bool tune_in_recommendation_cfg::tune_one_mk_sequence(
    ...
    for (const auto &recom_cfg : recommendation_cfg) {
        smap_t tuning_params;
        for (const auto &tune_para : tunning_para_name_list) {
            if (recom_cfg.find(tune_para) != recom_cfg.end()) {
                tuning_params[tune_para] = recom_cfg.at(tune_para);
            } else {
                std::cout << tune_para
                          << " is not found in recommendation tune result! If "
                             "you do not need this parameter, please ignore "
                             "this message."
                          << std::endl;
