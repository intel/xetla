...
bool cmdline_parser::parse_user_cmdline(map<string, string> &cmdline_info) {
    ...
    for (const auto &mk_name : operation_list) {
...
bool cmdline_parser::check_cmd() {
    ...
    for (const auto &option :
            usr_cmdline_info.usr_cfg_kernel_attr.kernel_attr) {
...
bool cmdline_parser::parse_common_format_usr_cmd_info(
    ...
    cur_delimit = ':';
    for (const auto &cfg_attr : cfg_para_list) {
        vector<string> attr;
...
bool cmdline_parser::parse_gemm_usr_cmd_info(
    ...
    for (const auto &shape : all_shape) {
...
void cmdline_parser::print_user_cmd_info() {
    cout << "user cmd:\noperation:\n";
    for (const auto &oper : usr_cmdline_info.mk_list) {
        cout << micro_kernel_type_map[oper] << ",";
    }
    cout << "\n";

    cout << "cfg attr:\n";
    for (const auto &cfg : usr_cmdline_info.usr_cfg_kernel_attr.kernel_attr) {
